...
def download_data():
    ...
    src = os.path.join(clone_dir, sub_folder)
    data_folder = base_data_folder.joinpath(sub_folder)
    if os.path.exists(data_folder):
        shutil.rmtree(data_folder)
    shutil.copytree(src, data_folder)
    shutil.rmtree(clone_dir)
...
def import_node_from_file(file_path, group, label=None, filename=None, upf=True):
    ...
    if label not in labels:
        try:
            ...
            if upf:
                node = orm.UpfData(file_path, filename=filename)
            else:
